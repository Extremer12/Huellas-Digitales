  ...
  const fetchAnimals = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from("animals").select("id, name, type, age, size, location, description, image_url, health_info, personality, user_id, status, created_at").eq("status", "disponible").order("created_at", {
        ascending: false
      });
      ...
      <section id="animales" className="section-padding bg-gradient-to-b from-background to-card scroll-reveal">
        <div className="container-custom">
          ...
          <div className="max-w-4xl mx-auto mb-12">
            ...
            {showFilters && (
              <div className="grid md:grid-cols-3 gap-4 p-6 bg-card/50 rounded-xl border border-primary/20">
                <div>
